...
const workflowStepLabels: Record<WorkflowStepKey, { label: string; owner: string; description: string }> = {
  UPLOAD_PROFILE: { label: "Prepare dataset", owner: "System", description: "Internal system step: register the dataset and build its deterministic aggregate profile before the agent starts." },
  UNDERSTAND_DATA: { label: "Understand data", owner: "Agent", description: "Turn the deterministic profile, schema and metadata into a semantic contract." },
  PROPOSE_RULES: { label: "Propose rules", owner: "Agent", description: "Generate typed rules with evidence and confidence." },
  REVIEW_RULES: { label: "Review rules", owner: "Steward", description: "Approve, request changes or reject the rule set." },
  PROPOSE_CODE: { label: "Propose standardization", owner: "Agent", description: "Create a deterministic code or transformation plan." },
...
function WorkflowPage({
  ...
  const renderArtifact = () => {
    ...
    if (currentArtifact.type === "SEMANTIC_CONTRACT") {
      ...
      return <div className="understanding-holder">
        <div className="understanding-summary"><span className="eyebrow">DATA UNDERSTANDING</span><p>{String(payload.summary ?? "Agent has not supplied a summary yet.")}</p></div>
        ...
        <div className="understanding-section"><div className="panel-heading"><div><span className="eyebrow">PROFILE EVIDENCE</span><h3>Signals used by Agent</h3></div></div><div className="evidence-list">{Array.isArray(payload.evidence) && payload.evidence.map((evidence) => <span key={String(evidence)} className="evidence-chip">{String(evidence)}</span>)}</div></div>
      </div>;
    }
  ...
  const nextActionLabel = currentStep?.key === "UPLOAD_PROFILE" ? "Prepare dataset" : currentStep?.key === "UNDERSTAND_DATA" ? "Run agent understanding" : currentStep?.key === "PROPOSE_RULES" ? "Generate rule proposals" : currentStep?.key === "PROPOSE_CODE" ? "Generate standardization code" : "Run current step";
  const visibleWorkflowSteps = workflow.steps.filter((step) => step.key !== "UPLOAD_PROFILE");
  const currentStepIndex = currentStep ? visibleWorkflowSteps.findIndex((step) => step.key === currentStep.key) : -1;
  const nextWorkflowStep = currentStepIndex >= 0 ? visibleWorkflowSteps[currentStepIndex + 1] : undefined;
    ...
    <>
      <div className="page-heading">
        <div>
          ...
          <p>{pipelineMode ? "Accept, edit, reject or add a manual rule. Agent stays locked until this set is approved." : "Review agent suggestions or author a typed rule manually."}</p>
        </div>
