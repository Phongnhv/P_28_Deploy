...
import type { AuditLog, CreateJobResponse, Dataset, DatasetProfile, DqResult, DqRun, Job, RuleProposal, RuleSpec } from "./types";
...
function LoginScreen({ onLogin, busy, error }: { onLogin: (password: string) => void; busy: boolean; error: string }) {
  const [password, setPassword] = useState("");
  const submit = (event: FormEvent) => { event.preventDefault(); onLogin(password); };
  return <main className="login-shell">
    ...
    <section className="login-card"><div className="mobile-brand"><span className="brand-mark">RP</span> RidePulse <em>DQ</em></div><span className="eyebrow">STEWARD ACCESS</span><h2>Welcome back</h2><p className="muted">Enter the shared demo password to open your workspace.</p><form onSubmit={submit}><label htmlFor="password">Demo password</label><input id="password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Enter password" autoFocus />{error && <div className="inline-error">{error}</div>}<button className="button primary full" disabled={busy || password.length < 1}>{busy ? "Opening workspace…" : "Open workspace →"}</button></form><div className="login-note"><span className="lock-icon">⌁</span><span><strong>Local development mode</strong><br />Use <code>demo</code> with the local adapter. Production will use the secure session API.</span></div></section>
  </main>;
}

function App() {
  const [authenticated, setAuthenticated] = useState(() => sessionStorage.getItem("ridepulse.auth") === "true");
  ...
  const refreshWorkspace = useCallback(async () => {
    ...
    } catch (err) { if (err instanceof ApiError && err.status === 401) { clearApiSession(); sessionStorage.removeItem("ridepulse.auth"); setAuthenticated(false); } setError(getErrorMessage(err, "Unable to load workspace.")); }
  ...
  async function handleLogin(password: string) {
    ...
    try { await api.createSession(password); sessionStorage.setItem("ridepulse.auth", "true"); setAuthenticated(true); }
    catch (err) { setLoginError(getErrorMessage(err, "Unable to start session.")); }
    finally { setLoginBusy(false); }
  }

  async function handleLogout() { await api.deleteSession(); sessionStorage.removeItem("ridepulse.auth"); setAuthenticated(false); }
  ...
  return <div className="app-shell"><aside className="sidebar"><div className="brand-lockup"><span className="brand-mark">RP</span><span>RidePulse <em>DQ</em></span></div><div className="sidebar-label">WORKSPACE</div><nav>{(["overview", "rules", "runs", "audit"] as View[]).map((item) => <button key={item} className={`nav-item ${view === item ? "active" : ""}`} onClick={() => setView(item)}><span className="nav-icon">{item === "overview" ? "◈" : item === "rules" ? "✦" : item === "runs" ? "↗" : "≡"}</span>{item === "overview" ? "Overview" : item === "rules" ? "Rule proposals" : item === "runs" ? "DQ runs" : "Audit history"}{item === "rules" && proposals.some((proposal) => ["PROPOSED", "EDITED"].includes(proposal.status)) && <span className="nav-count">{proposals.filter((proposal) => ["PROPOSED", "EDITED"].includes(proposal.status)).length}</span>}</button>)}</nav><div className="sidebar-bottom"><div className="security-card"><span className="shield">✦</span><div><strong>Guardrails active</strong><small>Aggregate evidence only</small></div></div><button className="profile-button" onClick={() => void handleLogout()}><span className="avatar">DS</span><span><strong>Data Steward</strong><small>Sign out</small></span><span className="chevron">↗</span></button></div></aside><main className="main-content"><header className="topbar"><div className="breadcrumb"><span>Workspace</span><span>/</span><strong>{view === "overview" ? "Overview" : view === "rules" ? "Rule proposals" : view === "runs" ? "DQ runs" : "Audit history"}</strong></div><div className="topbar-actions"><span className={`mode-badge ${isMockMode ? "mock" : "live"}`}><span />{isMockMode ? "LOCAL MOCK ADAPTER" : "CONNECTED API"}</span><button className="icon-button" aria-label="Notifications">♢</button><button className="avatar mini" aria-label="Current user">DS</button></div></header><div className="page-container">{isMockMode && <div className="dev-banner"><span>Local development adapter</span><span>Results are deterministic fixtures until the Gate 2 backend is connected.</span><code>VITE_USE_MOCK_API=false</code></div>}{error && <div className="alert error"><strong>Action needs attention</strong><span>{error}</span>{retryAction && <button className="retry-button" onClick={() => { retryAction(); setError(""); }}>Retry</button>}<button onClick={() => { setError(""); setRetryAction(null); }}>×</button></div>}{toast && <div className="alert success"><strong>Done</strong><span>{toast}</span><button onClick={() => setToast("")}>×</button></div>}{activeJob && <ProgressPanel job={activeJob} title={activeJob.type === "INGEST_PROFILE" ? "Building dataset profile" : activeJob.type === "PROPOSE_RULES" ? "Generating rule proposals" : "Running approved checks"} />}{view === "overview" && <OverviewPage dataset={dataset} profile={profile} proposals={proposals} approvedRules={approvedRules.length} loading={loading} busy={Boolean(activeJob)} onStartAnalysis={() => void startAnalysis()} onRequestProposals={() => void requestProposals()} onNavigate={setView} />}{view === "rules" && <RulesPage proposals={proposals} profileReady={Boolean(profile)} busy={Boolean(activeJob)} onRequestProposals={() => void requestProposals()} onApprove={(id) => void reviewProposal(id, "approve")} onReject={(id) => void reviewProposal(id, "reject")} onEdit={setEditingProposal} onRun={() => void runApprovedRules()} />}{view === "runs" && <RunsPage activeRun={activeRun} results={dqResults} approvedCount={approvedRules.length} busy={Boolean(activeJob)} onRun={() => void runApprovedRules()} />}{view === "audit" && <AuditPage logs={auditLogs} />}</div></main>{editingProposal && <EditDialog proposal={editingProposal} onClose={() => setEditingProposal(null)} onSave={(input) => void saveEdit(input)} />}</div>;
}

function OverviewPage({ dataset, profile, proposals, approvedRules, loading, busy, onStartAnalysis, onRequestProposals, onNavigate }: { dataset?: Dataset; profile: DatasetProfile | null; proposals: RuleProposal[]; approvedRules: number; loading: boolean; busy: boolean; onStartAnalysis: () => void; onRequestProposals: () => void; onNavigate: (view: View) => void }) {
...
function RulesPage({ proposals, profileReady, busy, onRequestProposals, onApprove, onReject, onEdit, onRun }: { proposals: RuleProposal[]; profileReady: boolean; busy: boolean; onRequestProposals: () => void; onApprove: (id: string) => void; onReject: (id: string) => void; onEdit: (proposal: RuleProposal) => void; onRun: () => void }) {
  ...
  return <><div className="page-heading"><div><span className="eyebrow">HUMAN-IN-THE-LOOP</span><h1>Rule proposals</h1><p>Review evidence, adjust typed parameters and approve only what belongs in your quality contract.</p></div><button className="button primary" onClick={onRun} disabled={!approved.length || busy}>Run approved rules <span>→</span></button></div>{!profileReady ? <section className="empty-state"><div className="empty-illustration">✦</div><h2>Profile first, proposals second</h2><p>Complete the dataset analysis before asking the guarded Agent for proposals.</p></section> : !proposals.length ? <section className="empty-state"><div className="empty-illustration">✦</div><h2>No proposals yet</h2><p>The proposal job uses aggregate profile evidence and returns 2–5 typed rules.</p><button className="button primary" onClick={onRequestProposals} disabled={busy}>Generate proposals →</button></section> : <><div className="review-summary"><div><span className="eyebrow">REVIEW QUEUE</span><strong>{pending.length} awaiting decision</strong></div><div className="review-progress"><span style={{ width: `${proposals.length ? ((proposals.length - pending.length) / proposals.length) * 100 : 0}%` }} /></div><span>{approved.length} approved · {proposals.filter((proposal) => proposal.status === "REJECTED").length} rejected</span></div><div className="proposal-list">{proposals.map((proposal) => <ProposalCard key={proposal.id} proposal={proposal} onApprove={() => onApprove(proposal.id)} onReject={() => onReject(proposal.id)} onEdit={() => onEdit(proposal)} />)}</div></>}</>;
}

function ProposalCard({ proposal, onApprove, onReject, onEdit }: { proposal: RuleProposal; onApprove: () => void; onReject: () => void; onEdit: () => void }) { const pending = ["PROPOSED", "EDITED"].includes(proposal.status); const tone = proposal.status === "REJECTED" ? "danger" : proposal.status === "APPROVED" ? "success" : "warning"; return <article className={`proposal-card ${proposal.status.toLowerCase()}`}><div className="proposal-top"><div className={`rule-type ${proposal.rule.type}`}><span>✦</span>{proposal.rule.type.replaceAll("_", " ")}</div><StatusPill label={proposal.status} tone={tone} /><span className={`severity ${proposal.severity.toLowerCase()}`}>{proposal.severity} severity</span></div><div className="proposal-main"><div className="proposal-content"><h3>{proposal.title}</h3><p>{proposal.description}</p><div className="rule-code"><span>TYPE</span><code>{formatRule(proposal.rule)}</code></div></div><div className="confidence"><span>CONFIDENCE</span><strong>{Math.round(proposal.confidence * 100)}%</strong><div className="confidence-track"><span style={{ width: `${proposal.confidence * 100}%` }} /></div></div></div><div className="evidence-row"><span className="evidence-label">EVIDENCE</span><span>{proposal.evidence_summary}</span>{proposal.evidence_refs.map((ref) => <code key={ref}>{ref}</code>)}</div>{pending && <div className="proposal-actions"><button className="button ghost" onClick={onReject}>Reject</button><button className="button secondary" onClick={onEdit}>Edit</button><button className="button primary" onClick={onApprove}>Approve rule <span>→</span></button></div>}</article>; }

function RunsPage({ activeRun, results, approvedCount, busy, onRun }: { activeRun: DqRun | null; results: DqResult[]; approvedCount: number; busy: boolean; onRun: () => void }) { return <><div className="page-heading"><div><span className="eyebrow">READ-ONLY EXECUTION</span><h1>DQ runs</h1><p>Persisted checks from approved typed rules. Failed results expose bounded IDs only.</p></div><button className="button primary" onClick={onRun} disabled={!approvedCount || busy}>Run approved rules <span>→</span></button></div>{!activeRun ? <section className="empty-state"><div className="empty-illustration">↗</div><h2>No run yet</h2><p>Approve at least one proposal, then execute it through the read-only runner.</p><button className="button primary" onClick={onRun} disabled={!approvedCount || busy}>Run approved rules →</button></section> : <><div className="run-hero"><div><span className="eyebrow">LATEST RUN</span><h2>{activeRun.id}</h2><p>Created {formatTime(activeRun.created_at)} · {activeRun.rule_ids.length} approved rules</p></div><StatusPill label={activeRun.status} tone={activeRun.status === "SUCCEEDED" ? "success" : "info"} /></div>{activeRun.status === "SUCCEEDED" && <div className="stat-grid run-stats"><StatCard label="Checked rows" value={activeRun.total_checked.toLocaleString()} detail="Across approved checks" tone="blue" /><StatCard label="Failed rows" value={activeRun.total_failed.toLocaleString()} detail="Bounded result summary" tone="amber" /><StatCard label="Rules executed" value={`${activeRun.rule_ids.length}`} detail="Approved versions only" tone="green" /><StatCard label="Raw values" value="0" detail="Never returned to browser" tone="violet" /></div>}<div className="panel"><div className="panel-heading"><div><span className="eyebrow">RESULTS</span><h3>Rule outcomes</h3></div><span className="panel-caption">{results.length} checks</span></div>{results.length ? <div className="results-table"><div className="result-header"><span>RULE</span><span>STATUS</span><span>CHECKED</span><span>FAILED</span><span>FAILED IDS</span></div>{results.map((result) => <div className="result-row" key={result.rule_id}><strong>{result.rule_title}</strong><StatusPill label={result.status} tone={result.status === "PASS" ? "success" : "danger"} /><span>{result.checked_count.toLocaleString()}</span><strong className={result.failed_count ? "metric-warn" : ""}>{result.failed_count.toLocaleString()}</strong><code>{result.failed_row_ids.length ? result.failed_row_ids.join(", ") : "—"}</code></div>)}</div> : <div className="table-empty">The runner is preparing bounded results…</div>}</div></>}</>; }
