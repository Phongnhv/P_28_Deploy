...
export const Graph3Analytics: React.FC<{ anomalies: DqAnomaly[] }> = ({ anomalies }) => {
  const { t } = useI18n();

  // Group anomalies by the type the backend actually returned. An earlier version
  // hardcoded two categories and drew two fake slices (1 and 2) when there were no
  // anomalies at all -- the chart looked populated while holding nothing.
  const anomalyCounts = anomalies.reduce<Record<string, number>>((acc, item) => {
    const key = item.anomaly_type || "UNKNOWN";
    acc[key] = (acc[key] ?? 0) + 1;
    return acc;
  }, {});
  const pieData = Object.entries(anomalyCounts).map(([name, value], index) => ({
    name: name.replaceAll("_", " ").toLowerCase(),
    value,
    tone: index % 2 === 0 ? ("fail" as const) : ("warn" as const),
  }));
  ...
  return (
    <div className="analytics-page">
      <div className="analytics-metrics">
        <div className="analytics-metric">
          <div className="label">{t("runs.anomaliesFound")}</div>
          <div className={`value ${anomalies.length > 0 ? "warn" : "good"}`}>{anomalies.length}</div>
          <div className="hint">
            {anomalies.length > 0 ? t("analytics.driftSignals") : t("analytics.noAnomaly")}
          </div>
        </div>
        <div className="analytics-metric">
          <div className="label">{t("analytics.anomalyTypes")}</div>
          <div className="value">{pieData.length}</div>
          <div className="hint">{t("analytics.distinctFamilies")}</div>
        </div>
      </div>

      <div className="analytics-card">
        <h3>{t("analytics.anomalySeverity")}</h3>
        <div className="analytics-chart-frame">
          {pieData.length === 0 ? (
            <EmptyChart message={t("analytics.emptyAnomalies")} />
          ) : (
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
