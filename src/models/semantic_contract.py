...
class SemanticColumn(BaseModel):
    ...
    nullable_expected: bool = Field(
        default=True,
        description="Liệu cột này có được phép mang giá trị trống/null theo logic nghiệp vụ không."
    )
...
class TableSemanticContract(BaseModel):
    ...
    business_assumptions: list[str] = Field(default_factory=list, description="Các giả định nghiệp vụ được rút ra từ bảng.")
...
class DatasetSemanticContract(BaseModel):
    ...
    tables: dict[str, TableSemanticContract] = Field(
        ...
        description="Bản đồ các bảng và contract ngữ nghĩa tương ứng."
    )
