...
class InferredDictionaryColumn(BaseModel):
    ...
    nullable_expected: bool = Field(True, description="Có cho phép null hay không theo logic nghiệp vụ thông thường.")
...
class InferredDictionaryTable(BaseModel):
    ...
    description: str = Field("", description="Mô tả tóm tắt ý nghĩa và mục đích nghiệp vụ của bảng bằng tiếng Việt.")
    columns: list[InferredDictionaryColumn] = Field(
        default_factory=list, description="Danh sách mô tả chi tiết của từng cột trong bảng."
    )
