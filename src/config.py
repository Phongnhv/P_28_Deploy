...
class Settings(BaseSettings):
    ...
    object_storage_endpoint_url: str | None = Field(
        default_factory=lambda: None if os.getenv("APP_ENV") == "production" else "http://localhost:9000"
    )
    object_storage_access_key_id: str | None = Field(
        default_factory=lambda: None if os.getenv("APP_ENV") == "production" else "minioadmin"
    )
    object_storage_secret_access_key: str | None = Field(
        default_factory=lambda: None if os.getenv("APP_ENV") == "production" else "miniopassword"
    )
