...
from src.services.rule_store import get_run, list_rules, init_db
...
async def run_test():
    ...
    from src.models.semantic_contract import TableSemanticContract, SemanticColumn
    from src.models.rule_schemas import TableRuleProposal, ProposedRule, RuleType, RuleConfidence, Severity, DataQualityDimension, ProposalBasis
    ...
    mock_semantic_contract = TableSemanticContract(
        ...
        columns=[
            SemanticColumn(name="order_id", semantic_type="identifier", business_role="primary_key", nullable_expected=False, confidence=1.0, description="Mã đơn hàng"),
            SemanticColumn(name="customer_id", semantic_type="identifier", business_role="customer_id", nullable_expected=False, confidence=1.0, description="Mã khách hàng"),
            SemanticColumn(name="order_total", semantic_type="currency", business_role="transaction_amount", nullable_expected=False, confidence=1.0, description="Tổng số tiền thanh toán"),
            SemanticColumn(name="order_status", semantic_type="category", business_role="order_status", nullable_expected=False, confidence=1.0, description="Trạng thái đơn hàng"),
            SemanticColumn(name="created_at", semantic_type="timestamp", business_role="created_at", nullable_expected=False, confidence=1.0, description="Thời gian tạo đơn hàng")
        ],
        relationships=[],
    ...
    mock_proposal = TableRuleProposal(
        ...
        rules=[
            ProposedRule(
                ...
                confidence=RuleConfidence(overall=1.0, evidence_strength=1.0, business_support=1.0, sample_representativeness=1.0, explanation="Không có giá trị null nào được phát hiện."),
            ...
            ProposedRule(
                ...
                confidence=RuleConfidence(overall=0.9, evidence_strength=0.9, business_support=0.9, sample_representativeness=0.9, explanation="Giá trị quan sát tối thiểu là 45.0, phù hợp quy chuẩn."),
