...
from src.services.rule_store import get_engine, publish_approved_rules, ProposedRuleModel
from src.models.database import RuleProposalModel, JobModel
...
with Session(engine) as session:
    ...
    legacy_updated = session.query(ProposedRuleModel).filter(
        ProposedRuleModel.run_id == run_id,
        ProposedRuleModel.status == "PENDING"
    ).update({"status": "APPROVED"}, synchronize_session=False)
    
    # 3. Update status of rules in rules schema (RuleProposalModel) to APPROVED
    proposals_updated = session.query(RuleProposalModel).filter(
        RuleProposalModel.dataset_id == dataset_id,
        RuleProposalModel.status == "PROPOSED"
    ).update({"status": "APPROVED"}, synchronize_session=False)
