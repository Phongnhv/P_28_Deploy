...
def test_detect_anomalies_with_exclusions(test_db):
    ...
    with Session(test_db) as session:
        ...
        anom_run2 = AnomalyRunModel(
            id="anom_h2", execution_run_id="run_h2", decision="ANOMALY", score=0.9, severity="HIGH"
        )
        ...
        for i in range(3, 8):
            ...
            res_normal.append(
                DqResultModel(
                    ...
                    failed_count=5,
                    failed_row_ids="[]",
                    violation_rate=0.05 if i != 5 else 0.06,
                )
            )
