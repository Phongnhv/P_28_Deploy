...
def test_semantic_dataset_file_exists():
    ...
    csv_path = project_root / "data" / "yellow_tripdata_2025" / "semantic_data" / "yellow_tripdata_2025_semantic_50k.csv"
...
def test_semantic_direct_value_replacements_in_place():
    ...
    valid_vendors = ["Creative Mobile Technologies, LLC", "Curb Mobility, LLC", "Myle Technologies Inc", "Helix", "Unknown Vendor"]
    assert df["vendor_id"].isin(valid_vendors).all()

    # 2. rate_code_id thay bằng tên biểu giá
    valid_ratecodes = ["Standard rate", "JFK", "Newark", "Nassau or Westchester", "Negotiated fare", "Group ride", "Null/Unknown", "Unknown Ratecode"]
    assert df["rate_code_id"].isin(valid_ratecodes).all()

    # 3. payment_type thay bằng tên phương thức thanh toán
    valid_payments = ["Flex Fare trip", "Credit card", "Cash", "No charge", "Dispute", "Unknown", "Voided trip", "Invalid Payment (Dispute/Test)", "Unknown Payment"]
