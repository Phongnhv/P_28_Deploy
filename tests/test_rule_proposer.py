...
def test_build_coverage_requirements_uses_digest_evidence():
    """Checklist phải phủ evidence mà không tự bịa tên cột."""
    ...
    keys = {(item["column"], item["rule_type"]) for item in requirements}

    assert ("pickup_at", "NOT_NULL") in keys
    assert ("pickup_at", "FRESHNESS") in keys
    assert ("payment_type", "ACCEPTED_VALUES") in keys
    assert ("fare_amount", "RANGE") in keys
    assert ("pickup_at", "CROSS_FIELD_COMPARISON") in keys
    assert (None, "ROW_COUNT") in keys
    assert {item["column"] for item in requirements} <= {
        "pickup_at", "dropoff_at", "payment_type", "fare_amount", None
    }
